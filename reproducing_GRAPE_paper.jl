using QuantumOptics, ForwardDiff, Plots
...
# Print the tensor product operators
println("Tensor product operator (a ⊗ I):")
println(a_op)
println("Tensor product operator (I ⊗ σ_minus):")
println(σ_minus_op)
...
# Example usage of the displacement function
alpha = 1.0 + 1.0im
...
function calculate_grape_gradient(ρ_t, σ_t, alphas, betas)
    ...
    for i in eachindex(alphas)
        ...
        dU_qc_dbeta = (-im * (conj(betas[i]) * dagger(a_op) * σ_minus / 2 + betas[i] * a_op * dagger(σ_minus) / 2)) * displacement(betas[i], dagger(a_op) * σ_minus / 2)
        dU_q_dalpha = (-im * (conj(alphas[i]) * σ_minus / 2 + alphas[i] * dagger(σ_minus) / 2)) * displacement(alphas[i], σ_minus / 2)

        commutator = dU_qc_dbeta * ρ_t[i] - ρ_t[i] * dU_qc_dbeta
        beta_grad[i] = tr(dagger(σ_t[i]) * commutator)
        mid_ρ = U_qc * ρ_t[i] * dagger(U_qc)
        commutator = dU_q_dalpha * mid_ρ - mid_ρ * dU_q_dalpha
        mid_σ = U_qc * σ_t[i] * dagger(U_qc)
...
target_state = tensor(fockstate(b_fock, 0), spinup(b_spin))
